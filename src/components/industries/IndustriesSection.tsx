...
export const IndustriesSection: React.FC<IndustriesSectionProps> = ({
  ...
  return (
    <section id="industries" className="py-24 sm:py-32 bg-[#FFFFFF] border-t border-slate-200/80 relative">
      <div className="container-editorial">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {INDUSTRIES_SERVED.map((ind) => (
            <div
              key={ind.id}
              className="rounded-3xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all duration-300 p-8 flex flex-col justify-between space-y-6 shadow-sm hover:shadow-md"
            >
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {ind.keyIngredients.map((ing, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded-xl bg-white border border-slate-200 font-mono text-[11px] text-slate-800 shadow-2xs font-medium"
                      >
                        {ing}
                      </span>
                    ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {ind.supplyingFirms.map((f, idx) => (
                    <button
                      ...
                      className="px-3 py-1 rounded-full bg-white hover:bg-[#E0581E] text-slate-800 hover:text-white border border-slate-300 hover:border-[#E0581E] text-xs font-mono transition-colors cursor-pointer shadow-2xs font-semibold"
                    >
                      {f.firmName} →
                    </button>
                  ))}
