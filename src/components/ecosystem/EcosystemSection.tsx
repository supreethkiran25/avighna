...
export const EcosystemSection: React.FC = () => {
  ...
  return (
    <section id="ecosystem" className="py-24 sm:py-32 bg-[#070809] border-t border-white/[0.08] relative">
      <div className="container-editorial">
        ...
        {/* Operating Entities Interactive Layout with Fluid Rounded Glass Ergonomics */}
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-[#E0581E] font-bold px-3 py-1 rounded-full bg-[#E0581E]/10 border border-[#E0581E]/20">
                      {division.number}
                    </span>
                    <span className="px-3 py-1 rounded-full bg-white/[0.04] border border-white/10 font-mono text-[10px] uppercase text-[#E5B25D]">
                      {division.badge}
                    </span>
                  </div>

                  <h3 className="mt-3 font-serif text-xl sm:text-2xl font-bold text-[#F9F8F5]">
                    {division.name}
                  </h3>

                  <p className="mt-1 font-mono text-xs text-[#9DA3AF]">
                    {division.role}
                  </p>
          ...
          {/* Right Entity Dossier & Offerings Panel (7 cols) in Curved Glass Container */}
          <div className="lg:col-span-7 rounded-3xl bg-[#0E1116]/80 backdrop-blur-2xl border border-white/10 p-6 sm:p-8 space-y-6 shadow-2xl">
            
            {/* Entity Header */}
            <div className="pb-4 border-b border-white/[0.08] flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                {activeDivision.kannadaName && (
                  <p className="font-kannada text-xs text-[#E5B25D] mt-0.5">
                    {activeDivision.kannadaName}
                  </p>
                )}
              </div>
              ...
              {activeDivision.phone && (
                <a
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.04] border border-white/10 text-xs font-mono text-[#F9F8F5] hover:border-[#E0581E] transition-all hover:scale-105"
                >
            ...
            {/* Key Offerings List with Rounded Chips */}
            <div className="space-y-2.5">
              <span className="font-mono text-xs uppercase tracking-wider text-[#E5B25D] block font-semibold">
                Core Portfolio & Capabilities:
              </span>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs font-sans text-[#F9F8F5]/80">
                {activeDivision.keyOfferings.map((offering, idx) => (
                  <li key={idx} className="p-3 rounded-2xl bg-white/[0.02] border border-white/5 flex items-start gap-2.5">
                    <span className="w-2 h-2 rounded-full bg-[#E0581E] shrink-0 mt-1.5 shadow-[0_0_8px_#E0581E]" />
                    <span>{offering}</span>
                  </li>
                ))}
              </ul>
            ...
            {/* Principals / Representation Pills */}
            <div className="pt-4 border-t border-white/[0.08] space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {activeDivision.principalsOrPartners.map((principal, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/10 font-mono text-xs text-[#E5B25D]"
                  >
                    {principal}
                  </span>
                ))}
            ...
            <div className="p-3.5 rounded-2xl bg-white/[0.02] border border-white/5 font-mono text-xs text-[#9DA3AF]">
        ...
        <div className="mt-20 pt-12 border-t border-white/[0.08]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {GLOBAL_PRINCIPALS.map((principal) => (
              <div
                key={principal.id}
                className="p-5 rounded-3xl bg-[#0E1116]/80 backdrop-blur-xl border border-white/10 hover:border-[#E5B25D]/50 transition-all duration-300 space-y-3 flex flex-col justify-between shadow-lg hover:-translate-y-1"
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[10px] uppercase text-[#E0581E] font-semibold px-2.5 py-0.5 rounded-full bg-[#E0581E]/10 border border-[#E0581E]/20">
                      {principal.origin}
                    </span>
                    <span className="px-2.5 py-0.5 rounded-full bg-white/[0.04] font-mono text-[9.5px] uppercase text-[#E5B25D]">
                      {principal.badge}
                    </span>
                  </div>
                  ...
                  <p className="text-xs font-mono text-[#9DA3AF]">{principal.role}</p>
                ...
                {principal.accreditation && (
                  <p className="text-[10.5px] font-mono text-[#E5B25D] pt-2 border-t border-white/[0.04]">
                    {principal.accreditation}
                  </p>
                )}
              </div>
